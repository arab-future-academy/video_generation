...
def interleaved_freqs_cis(freqs: torch.Tensor) -> torch.Tensor:
    """
    Compatibility shim for newer ComfyUI versions where
    interleaved_freqs_cis() was removed.
    """
    freqs = freqs.reshape(*freqs.shape[:-1], -1, 2)
    return torch.view_as_complex(freqs.float())


def split_freqs_cis(freqs: torch.Tensor) -> torch.Tensor:
    return interleaved_freqs_cis(freqs)
